...
- [[plans/shell-first-cli-supervisor.md]] — Delivers a first-class Shell
  ...
  release-gate N-1.
- [[plans/uta-broker-futu.md]] — Futu (富途) Broker Pack. All 3 increments
  (read-only `FutuGatewayClient`/`FutuBroker`; order placement/modify/
  cancel/close with trade-unlock; first-class `BROKER_ENGINE` + UI preset
  registration) are complete and unit-tested. No FutuOpenD gateway or Futu
  account is available for live verification.
